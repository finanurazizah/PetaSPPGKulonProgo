...
map.getView().fit([12234934.029920, -892818.002359, 12286241.642029, -852210.007225], map.getSize());
...
var Title = new ol.control.Control({
    element: (() => {
        ...
        titleElement.innerHTML = '<h2 class="project-title">Tingkat Keterjangkauan Distribusi MBG Berdasarkan Jarak Tempuh</h2>';
        return titleElement;
    })(),
...
document.getElementsByClassName('search-layer-input-search')[0].placeholder = 'Cari SPPG atau Sekolah ...';
...
function toggleCustomLayers(status) {
    ...
    function toggleLayerRecursive(layer) {

        // kalau layer group
        if (layer.getLayers) {
            layer.getLayers().forEach(toggleLayerRecursive);
        }
    ...
    map.getLayers().forEach(toggleLayerRecursive);

    // render ulang layer list
